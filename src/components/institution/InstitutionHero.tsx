...
const InstitutionHero: React.FC<InstitutionHeroProps> = ({ institucion, isAdmin, onEdit, onScrollTo }) => {
  ...
  const stats = [
    { id: 'oferta', label: 'Carreras', value: institucion.total_carreras, sub: 'Programas', icon: GraduationCap, color: 'text-indigo-400' },
    { id: 'campus', label: 'Campus', value: institucion.total_campus, sub: 'Planteles', icon: MapPin, color: 'text-emerald-400' },
    { id: 'resumen', label: 'Matrícula', value: Number(institucion.matricula_total || 0).toLocaleString(), sub: 'Estudiantes', icon: Users, color: 'text-blue-400' },
    { id: 'estadisticas', label: 'Mujeres', value: `${calculatedStats.pctMujeres.toFixed(0)}%`, sub: 'Participación', icon: Award, color: 'text-pink-400' },
    { id: 'estadisticas', label: 'Eficiencia', value: `${calculatedStats.eficiencia.toFixed(0)}%`, sub: 'Terminal', icon: TrendingUp, color: 'text-orange-400' },
    { id: 'estadisticas', label: 'IPD', value: `${calculatedStats.ipd.toFixed(0)}%`, sub: 'Probabilidad', icon: Star, color: 'text-amber-400' }
  ];
  ...
  return (
    <div className="relative">
      <div className={`relative h-[600px] w-full overflow-hidden ${institucion.color_hex || 'bg-slate-900'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-full flex flex-col justify-end pb-24 relative z-10">
          <div className="flex flex-col md:flex-row md:items-end gap-8 mb-12">
            ...
            <div className="flex-1 text-white">
              <div className="flex flex-wrap gap-2 mb-4">
                <span className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${institucion.id_sostenimiento === 2 ? 'bg-blue-600' : 'bg-emerald-600'}`}>
              ...
              <motion.h1 
                ...
                className="text-4xl md:text-7xl font-black tracking-tighter leading-[0.9] mb-4 drop-shadow-2xl"
              >
                {institucion.nombre}
              </motion.h1>
              {institucion.siglas && (
            ...
            <div className="w-full md:w-72 bg-white/10 backdrop-blur-xl rounded-[32px] p-5 md:p-6 border border-white/10 text-white space-y-3 shadow-2xl">
              {institucion.sitio_web && (
                <a href={institucion.sitio_web} target="_blank" rel="noreferrer" className="flex items-center gap-3 group">
                  ...
                  <span className="text-xs md:text-sm font-bold truncate">{institucion.sitio_web.replace('https://', '').replace('http://', '')}</span>
                </a>
              )}
              {institucion.telefono && (
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs md:text-sm font-bold">{institucion.telefono}</span>
                </div>
              )}
              <div className="flex flex-col gap-2 pt-2">
                {isAdmin && (
                  <button 
                    onClick={onEdit}
                    className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 flex items-center justify-center gap-2"
                  >
                    <Award size={16} />
                    Editar Perfil
                  </button>
                )}
                <button className="w-full py-4 bg-white text-slate-900 rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest hover:bg-slate-100 transition-all shadow-xl">
                  Guardar
                </button>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
            {stats.map((stat, i) => (
              <motion.button 
                ...
                key={i} 
                onClick={() => onScrollTo?.(stat.id)}
                className="bg-white/5 backdrop-blur-md p-4 rounded-3xl border border-white/10 text-left transition-all hover:bg-white/20 hover:-translate-y-1 active:scale-95 group"
              >
                ...
                <div className="text-xl md:text-2xl font-black text-white">{stat.value || 'N/A'}</div>
